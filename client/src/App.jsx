...
const ProtectSellerDetail = ({ children }) => {
  ...
  if (!user) return <Navigate to="/login" replace />; // optional check

  if (user.isSeller && !user.isSellerVerification) {
    return children;
  }

  // if seller already verified → redirect
  if (
    user.isSeller &&
    user.isSellerVerification &&
    user.seller.isVerifiedSeller
  ) {
    return <Navigate to="/seller-page?tab=profile" replace />;
  }

  // if not a seller → redirect somewhere else (optional)
  return <Navigate to="/" replace />;
};
...
function App() {
  ...
  return (
    <div className={`flex flex-col  overflow-y-scroll scrollbar-hide  bg-gradient-to-br ${
          theme === "light"
