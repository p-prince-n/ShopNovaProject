...
const Cart = () => {
  ...
  console.log({selectedItemsData, selectedItems});
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedItems(cart.items.map((item) => item.product._id)); // add all IDs
      setSelectedItemsData(cart.items.map((item) => item));
    } else {
      setSelectedItems([]); // clear list
    ...
    if (navigator.canShare && navigator.canShare({ files: [] })) {
      ...
      canvas.toBlob(async (blob) => {
        ...
        try {
          ...
          generateQRDataUrl(); // generate preview
        } catch (err) {
          console.error(err);
