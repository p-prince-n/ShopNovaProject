...
import { useParams, useNavigate } from "react-router-dom"; // <-- for redirect
import { useProductStore } from "../../Store/useProductStore";
import { useAuthStore } from "../../Store/authStore"; // <-- assuming you have a user store
...
const ProductDetail = () => {
  ...
  // Fetch product and set userRating if logged-in user rated
  useEffect(() => {
    fetchProductById(id).then((prod) => {
      if (prod?.userRatings?.length > 0 && user) {
        ...
        if (myRating) {
          ...
          setUserRating(0); // user hasn't rated yet
        }
      } else {
        setUserRating(0); // no ratings at all
      }
    });
  ...
  const handleRating = async (newRating) => {
    ...
    setUserRating(newRating); // optimistic UI
  ...
  const getAveragerating = () => {
    if (userRating !== 0) {
      let avgrat = (
        (product?.ratings * product.ratingsCount + userRating) /
        (product.ratingsCount + 1)
      ).toFixed(1);
      return avgrat;
    }
    return product?.ratings.toFixed(1);
  };
...
const isInCart = cart?.items?.some((item) => {
  ...
  // ✅ if product has sizes → match both productId and size
  ...
  // ✅ if product has no sizes → match only productId
                          ...
                          >
                            <motion.button
                              whileHover={{
                                ...
                                background:
                                  theme === "dark"
                                    ? "linear-gradient(135deg, #166534, #22c55e, #4ade80)" // dark hover gradient (gray shades)
                                    : "linear-gradient(135deg, #f97316, #f59e0b)", // light hover gradient (orange/yellow)
                              }}
                              whileTap={{
                                scale: 0.9,
                                rotate: -2,
                                ...
                                background:
                                  theme === "dark"
                                    ? "linear-gradient(135deg, #065f46, #10b981)" // dark click gradient (green shades)
                                    : "linear-gradient(135deg, #f59e0b, #d97706)", // light click gradient (blue/purple)
                              }}
                              transition={{
                                type: "spring",
                                stiffness: 300,
                              ...
                              className={`size-18  rounded-lg shadow-2xl border-2 flex items-center justify-center ${
                                theme === "dark"
                                  ? "border-gray-800 text-white" // dark default bg
                                  : "border-orange-400 text-white bg-orange-500" // light default bg
                              }`}
                              style={{
                                background: `${
                                  theme === "dark"
