...
export const DiscountproductCard = ({ product }) => {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 60 }}   // 👈 scroll down animation
    ...
    >
      ...
      />
      <h3 className="font-medium text-gray-800 dark:text-gray-200 truncate w-40 overflow-ellipsis">
        {product.name}
      </h3>
...
const ProductCategoryDiscount = ({ categoryName, products, discount = true }) => {
  ...
  // Card animation variant (scroll up/down)
  const cardVariants = {
    hidden: { opacity: 0, y: 60 },
    visible: { opacity: 1, y: 0 },
    exit: { opacity: 0, y: 60 }, // 👈 animate down when leaving
  };
  ...
  return (
    <motion.div
      ...
      whileInView={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, y: 80 }}   // 👈 animate back down
