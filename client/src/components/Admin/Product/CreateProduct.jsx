...
const CreateProduct = ({ product = null, updatePage = null }) => {
  ...
  // Add uploaded image to productData when imageUrl changes
  useEffect(() => {
    if (imageUrl) {
      setProductData((prev) => ({
        ...prev,
  ...
  const handleUploadImages = async (files) => {
    ...
    for (let file of selectedFiles) {
      ...
      await uploadImage(file); // `useEffect` will add image to productData
  ...
  const handleSubmit = async (e) => {
    ...
    // Regex to check if input is a number (integer or decimal)
    ...
    >
        ...
        <div className="flex w-full items-center justify-center">
          ...
          >
            ...
            <FileInput
              ...
              className="hidden"
              multiple
              onChange={(e) => handleUploadImages(e.target.files)} // use the function for multiple files
            />
          </Label>
        </div>
      </div>
      ...
      <div className="md:px-10 mt-4">
        ...
        <input
          ...
          value={(productData.famousInCities || []).join(", ")}
          onChange={(e) =>
            setProductData({
              ...productData,
              famousInCities: e.target.value
                .split(",")
                .map((c) => c.trim())
                .filter((c) => c), // optional: remove empty strings
            })
          }
          className={`w-full px-3 py-2 rounded-lg border ${
            theme === "dark"
