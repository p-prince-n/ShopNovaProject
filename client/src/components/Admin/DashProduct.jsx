...
const DashProducts = () => {
  ...
  // ✅ Load first batch
   ...
   useEffect(() => {
    const handler = setTimeout(() => {
      ...
      if (search.trim() !== "") {
        params.set("search", search); // update the search param
        searchProducts(search); // fetch filtered categories
      } else {
        params.delete("search"); // remove param if empty
        fetchProducts(0); // fetch all categories
      }
  ...
  }, [search, fetchProducts,getProducts,user?.isAdmin, navigate, location.pathname, location.search]);

  // ✅ Load more
  const handleShowMore = async () => {
    await fetchProducts(products.length); // append more
  };
  ...
  return (
    ...
    <div className=" py-5 w-full table-auto overflow-x-scroll md:mx-auto scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500">
      {user?.isAdmin && products.length > 0 ? (
            ...
            <TableBody className="divide-y">
              {products.map((p) => (
                ...
                >
                  ...
                  <TableCell className="whitespace-nowrap">{p.discount}%</TableCell>
                  <TableCell className="whitespace-nowrap">{p.stockQuantity}</TableCell>
                  <TableCell className="whitespace-nowrap">{p.ratings}/5</TableCell>
                  <TableCell className="whitespace-nowrap">
                    <Link
        ...
        <p className="dark:text-green-400">{search? "Product Not found":"You have no products yet!"}</p>
      )}
