...
const SellerProductDetalsPage = ({
  item,
  order,
  HandleBackButton,
  ship = false,
}) => {
  const { sellerAcceptOrder,sellerShipOrder,fetchSellerPendingOrders, fetchSellerShippingPendingOrders} =
    useOrderStore();
    ...
    <div className="bg-transparent w-full min-h-screen flex flex-col items-center  relative overflow-y-scroll scrollbar-hide">
      <div className={`${fullSizeImage ? "backdrop-blur-md" : ""} `}>
        <div className="w-full flex flex-col gap-2 ">
          <div className="w-full flex flex-col xl:flex-row items-center gap-2 md:gap-5 xl:gap-10">
          ...
          <div className="flex items-center justify-center w-full">
            <Button
              onClick={async () => {
                ...
                if (!ship) {
                  await sellerAcceptOrder(order?._id, item?.product?._id);
                  
                  toast.success("this order accepted by you");
                  
                }else{
                  await sellerShipOrder(order?._id, item.product?._id)
                  
                  toast.success("this order Shipped by you");
                }
                await fetchSellerPendingOrders();
                await fetchSellerShippingPendingOrders();
                HandleBackButton();
              }}
...
export default SellerProductDetalsPage;
