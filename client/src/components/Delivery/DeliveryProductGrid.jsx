...
const DeliveryProductGrid = ({ item, selectItem, order, selectOrder, pickupProduct = false, verified=false }) => {
  return (
    <>
      <div className="hidden md:w-1/2 lg:w-2/7 2xl:w-1/6 md:flex gap-3 mt-5">
        <ProductCard item={item} selectItem={selectItem} order={order} selectOrder={selectOrder} verified={verified} pickupProduct={pickupProduct} />
      </div>
      <div className="flex flex-col items-center justify-center gap-2 md:hidden px-1 mt-5">
        <ProductCardMobile item={item} selectItem={selectItem} order={order} selectOrder={selectOrder} verified={verified} pickupProduct={pickupProduct} />
      </div>
    </>
  );
};
...
const ProductCardMobile = ({ item, selectItem, order, selectOrder, pickupProduct , verified }) => {
  return (
    <motion.div
      onClick={() => { selectItem(item); selectOrder(order); }}
      className="flex border-[0.5px] w-[60vw] items-center justify-between gap-5 bg-white dark:bg-gray-900 p-2 rounded-lg cursor-pointer shadow-sm md:hidden"
      whileHover={{
        scale: 1.03,
    ...
    >
      ...
      <span className="text-gray-400 dark:text-gray-300 text-xl flex-1/8">›</span>
    </motion.div>
  );
};


const ProductCard = ({ item, selectItem, order,selectOrder, pickupProduct , verified }) => {
    ...
    >
      ...
      <div className="flex flex-col p-2 flex-1">
        ...
        <div className="mt-auto">
          <p className="font-semibold text-purple-600 dark:text-purple-400 text-sm flex-col">
          ...
          >
            {verified ? 'Deliverd' : 'Assign Delivery'}
          </motion.button>
        </div>
      </div>
    </motion.div>
...
export default DeliveryProductGrid;
