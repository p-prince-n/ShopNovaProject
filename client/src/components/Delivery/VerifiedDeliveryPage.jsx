...
import DeliveryProductDetailsPage from "./DeliveryProductDetailsPage";
import { ArrowLeftCircle, PackageX } from "lucide-react";

const VerifieddeliveryPage = () => {
  const { loading, deliveryPendingOrders, fetchDeliveryPendingOrders } =
    useOrderStore();
  ...
  useEffect(() => {
    fetchDeliveryPendingOrders();
  }, [fetchDeliveryPendingOrders]);


  const items = deliveryPendingOrders.flatMap((order) => order.items);
  ...
  const SelectTheItem = (item, order) => {
    ...
    <div className="w-full h-full px-2 md:px-10 py-3 md:py-5">
      ...
      {items.length > 0 ? (
        selectedItem === null ? (
            ...
            {deliveryPendingOrders.map((order) =>
              order.items.map((item, idx) => (
                <DeliveryProductGrid
                  ...
                  order={order}
                  item={item}
                  selectItem={(it) => SelectTheItem(it, order)}
          ...
          <>
            <button onClick={HandleBackButton} className="mb-4">
              <ArrowLeftCircle className="dark:text-blue-400 w-7 h-7" />
            </button>
            <div>
        ...
        >
          ...
          >
            There are currently no shipped orders waiting to be picked up. Once
            sellers ship products, they will appear here.
          </motion.p>

          <motion.button
            onClick={fetchDeliveryPendingOrders}
...
export default VerifieddeliveryPage;
