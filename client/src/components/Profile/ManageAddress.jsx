...
const ManageAddress = () => {
  ...
  const HandleSubmitAddress = async () => {
    ...
    if (showUpdateAddress) {
      ...
      setShowAddAddress(false); // close modal after update
    } else {
  ...
  const handleEditClick = (addr) => {
    setSelectedAddress(addr); // store selected address object
    setAddress({
      roomNo: addr.roomNo || "",
      street: addr.street || "",
      city: addr.city || "",
    ...
    setShowUpdateAddress(true); // open modal
    setShowAddAddress(true);
  };

  // ⭐ Added: Get current location & autofill address
