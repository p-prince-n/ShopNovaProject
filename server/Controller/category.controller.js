...
export const createCategory = async (req, res) => {
  try {
    ...
    const image = req.body.image || null; // assuming using multer + cloudinary
...
export const updateCategory = async (req, res) => {
  try {
    ...
    // If parentCategory is null or empty, remove it
...
export const searchCategory = async (req, res) => {
  try {
    ...
    // If the value is a valid ObjectId, also check parentCategory
    if (/^[0-9a-fA-F]{24}$/.test(value)) {
      query.$or.push({ parentCategory: value });
    }

    // Fetch full category data + parentCategory details
